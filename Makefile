# variables
DB_CONTAINER=zesty-db
BACKEND_CONTAINER=zesty-backend
...
.PHONY: help build run clean deps tidy docker-build docker-up docker-down docker-restart docker-logs-backend docker-logs-db docker-exec-db db-up db-down db-reset migrate-up migrate-down seed-up seed-down stop-all
...
docker-exec-db: ## Execute MySQL shell in database container
	docker exec -it $(DB_CONTAINER) mysql -u root -p$(shell grep MYSQL_ROOT_PASSWORD .env | cut -d '=' -f2)
...
db-reset: ## Reset database (WARNING: This will delete all data)
	@echo "$(RED)[!] WARNING: This will delete all database detai!$(NC)"
...
seeds-up: ## Apply all seed UP SQL files (non-migrate)
	...
	migrate -path ./backend/db/seeds -database "$(DB_URL)" up
...
seeds-down: ## Run ALL DOWN migrations (destructive)
	...
	migrate -path ./backend/db/seeds -database "$(DB_URL)" down
